...
export const getComponentResourceType = (type: string) => `drunk-pulumi:index:${type}`;

export interface BaseArgs extends WithVaultInfo, WithResourceGroup { }

export class BaseComponentResource<TArgs extends BaseArgs> extends pulumi.ComponentResource {
    private secrets: { [key: string]: pulumi.Input<string> } = {};
    private _vaultSecretsCreated: boolean = false;
    private vaultSecrets?: { [key: string]: VaultSecretResult };

    constructor(private type: string, private name: string, private args: TArgs, opts?: pulumi.ComponentResourceOptions) {
    ...
    protected getRsGroupInfo(): ResourceGroupInfo {
        ...
        if (rsGroup instanceof ResourceGroup) return { resourceGroupName: rsGroup.name, location: rsGroup.location };
        return rsGroup;
    }
    ...
    protected registerOutputs(outputs?: pulumi.Inputs | Promise<pulumi.Inputs> | pulumi.Output<pulumi.Inputs>): void {
        this.postCreated();
